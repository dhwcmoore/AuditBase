    ...
    let rec walk acc = function
      ...
      | l::ls when starts_with l "- range:" ->
          ...
          let cat =
            ...
            | Some l2 ->
                let t2 = trim l2 in
                let c = trim (String.sub t2 9 (String.length t2 - 9)) in
...
(* ---------- run one (supports --json, returns bool) ---------- *)
let run_one ?(json=false) name =
  ...
  if json then begin
    let failures_json =
      ...
      |> String.concat ","
    in
    Printf.printf {|{"id":"'%s'","domain":"%s","monotonicity":%b,"failures":[%s]}|} name dn mono_ok failures_json;
    print_newline ();
  end else begin
    Printf.printf "== Audit: %s (id: %s) ==\n" dn name;
